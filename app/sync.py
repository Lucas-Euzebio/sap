...
def sync_invoices(since_date=None):
    ...
    if not session_id:
        raise Exception("Falha ao autenticar no SAP Service Layer. Verifique as credenciais.")

    sap_url = get_sap_url()
    endpoint = f"{sap_url}/b1s/v1/Invoices"
    ...
    while url_atual:
        response = requests.get(url_atual, headers=headers, params=params if url_atual == endpoint else None, cookies={"B1SESSION": session_id}, verify=False)
        if response.status_code != 200:
            raise Exception(f"Erro ao consultar faturas no SAP: {response.status_code} - {response.text}")

        data = response.json()
        contas = data.get("value", [])
        print(f"📥 Processando página com {len(contas)} faturas em aberto...")
        ...
        if next_link:
            url_atual = f"{sap_url}/b1s/v1/{next_link}" if not next_link.startswith("http") else next_link
        else:
            url_atual = None
...
def sync_recebidas(since_date=None):
    ...
    if since_date:
        # Busca 30 dias antes da data solicitada para garantir captura de pagamentos retroativos
        base_dt = datetime.strptime(since_date, '%Y-%m-%d')
        safe_date = (base_dt - timedelta(days=30)).strftime('%Y-%m-%d')
        filters.append(f"DocDate ge '{safe_date}'")
    else:
        # Se for primeira carga, traz os últimos 30 dias por padrão
        safe_date = (datetime.now() - timedelta(days=30)).strftime('%Y-%m-%d')
        filters.append(f"DocDate ge '{safe_date}'")
    ...
    while url_atual:
        response = requests.get(url_atual, cookies={"B1SESSION": session_id}, headers=headers, params=params if url_atual == endpoint else None, verify=False)
        if response.status_code != 200:
            raise Exception(f"Erro ao consultar recebimentos no SAP: {response.status_code} - {response.text}")

        data = response.json()
        pagamentos = data.get("value", [])
        print(f"📥 Processando página com {len(pagamentos)} recebimentos...")
